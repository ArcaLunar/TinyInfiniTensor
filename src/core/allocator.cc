...
void Allocator::free(size_t addr, size_t size) {
  ...
  // first, insert
  auto it = free_blocks.insert(std::pair{addr, size})
                .first; // iterator to the newly inserted block

  // second, find if the newly released block can be merged with existing free
  // blocks
  // 1. look to its left
  ...
  // 3. if the freed block is at the end, move back 'used'
  auto end = free_blocks.rbegin();
  if (end->first + end->second == used) {
    used -= end->second;
