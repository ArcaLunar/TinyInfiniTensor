...
namespace infini {
  ...
  OperatorObj::OperatorObj(OpType opType, TensorVec inputs, TensorVec outputs) : type(opType), inputs(inputs), outputs(outputs) {}

  void OperatorObj::removePredecessors(const Operator &op) {
    for (auto it = predecessors.begin(); it != predecessors.end();) {
      if (it->lock() == op) it = predecessors.erase(it);
      else ++it;
    }
  }
  ...
  void OperatorObj::removeSuccessors(const Operator &op) {
    for (auto it = successors.begin(); it != successors.end();) {
      if (it->lock() == op) it = successors.erase(it);
      else ++it;
    }
  }
  ...
  bool OperatorObj::checkValid(GraphObj *graph) {
    ...
    if (shapes.size() != outputs.size()) return false;
    if (graph) { // if graph != nullptr, outputs should be created
      ...
      for (size_t i = 0; i < shapes.size(); ++i) {
        if (shapes[i] != outputs[i]->getDims()) return false;
      }
    }
    return true;
  }
  ...
  vector<DataType> OperatorObj::inferDataType() const { return inferDataType(inputs); }
