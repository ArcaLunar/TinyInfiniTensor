...
#define _IT_ASSERT_2(condition, info) static_cast<bool>(condition) ? void(0) : throw ::infini::Exception(std::string("[") + __FILE__ + ":" + std::to_string(__LINE__) + "] Assertion failed (" + #condition + "): " + info)
...
#define IT_TODO_SKIP() puts("Unimplemented " __FILE__ ":" __LINE__)
  ...
  template <typename T>
  std::string vecToString(const T *st, size_t length) {
    std::stringstream ss;
    ss << "[";
    size_t i = 0;
    for (i = 0; i < length; i++) {
