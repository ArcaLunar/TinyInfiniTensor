...
namespace infini {
  ...
  private:
    ...
    string dataToString() const {
      ...
      for (int i = numDims - 1; i != 0; --i) dimSzVec[i - 1] = dimSzVec[i] * shape[i - 1];
      ...
      for (size_t i = 0; i < size; ++i) {
        if constexpr (std::is_integral_v<T>) {
          ...
          if (std::min(fabs(a[i]), fabs(b[i])) == 0. && fabs(a[i] - b[i]) > relativeError) {
            ...
            return false;
          } else if (std::min(fabs(a[i]), fabs(b[i])) != 0. && fabs(a[i] - b[i]) / std::max(fabs(a[i]), fabs(b[i])) > relativeError) {
            printf("Error on %lu: %f %f\n", i, a[i], b[i]);
            return false;
          }
        } else {
    ...
    void removeTarget(const Operator &op) {
      for (auto itr = targets.begin(); itr != targets.end();) {
        if (itr->lock() == op) itr = targets.erase(itr);
        else ++itr;
      }
    }
  };
